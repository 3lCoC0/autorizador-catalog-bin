...
public record AgencyCreateRequest(
        ...
        @NotNull(message = "agencyCode no puede ser nulo")
        @NotBlank(message = "agencyCode no puede ser vacio")
        @Size(min = 2, max = 2,message="agencyCode debe ser de longitud de 2 posiciones")
        @Pattern(regexp="\\d*", message="agencyCode debe ser numérico")
        @Pattern(regexp = "^[\\p{L}\\p{N}\\s]+$", message = "agencyCode no debe contener caracteres especiales") String agencyCode,

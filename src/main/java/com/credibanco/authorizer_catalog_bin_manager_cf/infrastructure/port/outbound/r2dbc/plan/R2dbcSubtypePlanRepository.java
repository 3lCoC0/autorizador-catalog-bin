...
public class R2dbcSubtypePlanRepository implements SubtypePlanRepository {
    ...
    public Mono<Integer> upsert(String subtypeCode, Long planId, String by) {
        ...
        log.debug("Repo:SUBTYPE_PLAN:upsert:start subtype={} planId={}", subtypeCode, planId);
        ...
        """).bind("sc", subtypeCode).bind("pid", planId);
        spec = (by == null) ? spec.bindNull("by", String.class) : spec.bind("by", by);

        return spec.fetch().rowsUpdated()
                .doOnNext(n -> log.debug("Repo:SUBTYPE_PLAN:upsert:rowsUpdated subtype={} n={}", subtypeCode, n))
                .map(Long::intValue)
                .doOnSuccess(n -> log.info("Repo:SUBTYPE_PLAN:upsert:done subtype={} affected={} elapsedMs={}",
                        subtypeCode, n, ms(t0)))
                .doOnError(e -> log.warn("Repo:SUBTYPE_PLAN:upsert:error subtype={} planId={}", subtypeCode, planId, e));
    }

    @Override
    public Mono<SubtypePlanLink> findBySubtype(String subtypeCode) {
        long t0 = System.nanoTime();
        log.debug("Repo:SUBTYPE_PLAN:findBySubtype:start subtype={}", subtypeCode);
        ...
        """).bind("sc", subtypeCode)
                .map(MAPPER).one()
                .doOnSuccess(link -> log.debug("Repo:SUBTYPE_PLAN:findBySubtype:hit subtype={} elapsedMs={}", subtypeCode, ms(t0)))
                .doOnError(e -> log.warn("Repo:SUBTYPE_PLAN:findBySubtype:error subtype={}", subtypeCode, e));
    }
}
